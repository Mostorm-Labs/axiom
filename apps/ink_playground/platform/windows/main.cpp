...
LRESULT CALLBACK WindowProc(HWND window, UINT message, WPARAM wParam, LPARAM lParam) {
  ...
  if (message == WM_POINTERDOWN || message == WM_POINTERUPDATE || message == WM_POINTERUP) {
    ...
    const UINT pointerId = GET_POINTERID_WPARAM(wParam); POINTER_INFO info{};
    if (!GetPointerInfo(pointerId, &info)) return 0;
    float pressure = 0.5F; POINTER_PEN_INFO pen{};
    if (info.pointerType == PT_PEN && GetPointerPenInfo(pointerId, &pen))
      pressure = static_cast<float>(pen.pressure) / 1024.0F;
    ...
    arc_pointer_sample_v0 sample{}; sample.pointer_id = pointerId; sample.sample_sequence = info.dwTime;
    sample.timestamp_us = static_cast<std::uint64_t>(info.dwTime) * 1000U;
    sample.x = static_cast<float>(info.ptPixelLocation.x); sample.y = static_cast<float>(info.ptPixelLocation.y);
    sample.pressure = std::clamp(pressure, 0.0F, 1.0F); sample.phase = phase;
    ...
    batch.view_id = 1; batch.viewport_revision = 1; batch.device_id = info.sourceDevice;
    ...
    batch.samples = &sample; batch.sample_count = 1; batch.sample_stride = sizeof(sample);
    value->input->SubmitBatch(batch); if (phase == ARC_POINTER_PHASE_UP) value->host->commitStroke(value->stroke, value->stroke);
...
int WINAPI WinMain(HINSTANCE instance, HINSTANCE, LPSTR, int show) {
  ...
  State value; value.host = std::make_unique<InkPlaygroundHost>(); value.host->bindSurface(1024, 768);
