...
LocalityWorkloadResult runControlledCascadeWorkload(std::size_t affected_closure_size){constexpr std::size_t total=100000U;IndexedObjectStore s;static_cast<void>(internal::ObjectStoreMutator::insertFresh(s,shape(1)));const auto connector_count=affected_closure_size-1U;for(std::size_t i=0;i<connector_count;++i)static_cast<void>(internal::ObjectStoreMutator::insertFresh(s,connector(100+i,ObjectId::fromUint64(1))));populateCold(s,total-affected_closure_size,1000000U);return applyWorkload("W08-CONTROLLED-CASCADE",total,affected_closure_size,s,DeleteObjectsOp{{ObjectId::fromUint64(1)}},8100+affected_closure_size);}
}
