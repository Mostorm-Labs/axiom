...
ImpactClassification classifyImpact(const semantic::ObjectSemanticChange& change) noexcept {
    ...
    if (has(F::kCreated) || has(F::kDeleted)) {
        out.record = out.world_bounds = out.spatial = out.hierarchy = DirtyState::kDirty;
        return out;
    }
    if (has(F::kPlacement)) {
    ...
    if (has(F::kProperties)) {
        ...
        for (const auto field : change.changed_fields) opacity_only = opacity_only && field == 3U;
        if (!opacity_only) out.visual_bounds = out.world_bounds = out.spatial = DirtyState::kDirty;
        else out.visibility = DirtyState::kDirty;
    }
    return out;
}
} // namespace canvas::scene
