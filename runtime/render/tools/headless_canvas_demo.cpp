...
int main(int argc,char**argv){
 ...
 auto vis=render::VisibilityResolver::resolve(frame,*scene);if(!vis)return fail("visibility failed");auto query=scene->query(SceneQuery{frame.worldViewport});if(!query)return fail("query failed");auto draw=scene->buildDrawList(query.value());if(!draw)return fail("draw list failed");auto refs=render::DirectReferenceSource::build(frame,vis.value(),coordinator.runtimeScene());if(!refs)return fail("reference failed");auto plan=render::FramePlanBuilder::build(frame,refs.value());if(!plan)return fail("plan failed");RecordingBackend backend({256,256});if(render::FrameOrchestrator::submit(backend,plan.value()).code!=render::BackendSubmissionCode::kAccepted||!backend.backend().observation())return fail("render failed");const auto& obs=*backend.backend().observation();const auto groupId=foundation::ObjectId::fromUint64(509U);const bool groupAbsentFromPixelStages=std::find(vis.value().backToFront.begin(),vis.value().backToFront.end(),groupId)==vis.value().backToFront.end()&&std::none_of(draw.value().items.begin(),draw.value().items.end(),[&](const auto& item){return item.objectId==groupId;})&&std::none_of(refs.value().entries.begin(),refs.value().entries.end(),[&](const auto& entry){return entry.record.objectId==groupId;})&&std::find(backend.backendIds().begin(),backend.backendIds().end(),groupId)==backend.backendIds().end();
 fs::path parent=output.parent_path();if(parent.empty())parent=".";fs::path stage=parent/(output.filename().string()+".g3-07-staging");std::error_code ec;fs::remove_all(stage,ec);if(!fs::create_directory(stage,ec))return fail("staging failed");if(!writeFile(stage/"render.rgba",obs.rgba)){fs::remove_all(stage,ec);return fail("raster write failed");}{std::ofstream d(stage/"render-digest.txt");d<<obs.digest<<'\n';}{std::ofstream e(stage/"render-evidence.json");e<<"{\"result\":\"PASS\",\"fixture_id\":\"g3-07-nine-kind-v1\",\"fixture_bytes\":"<<bytes.size()<<",\"fixture_sha256\":\""<<sha256(bytes)<<"\",\"fixture_digest\":\""<<fnv(bytes)<<"\",\"pipeline_stages\":[\"SnapshotCodec.decode\",\"SnapshotBootstrapper.restore\",\"IndexedObjectStore\",\"FullSceneCompiler\",\"Scene\",\"RuntimeScene\",\"VisibilityResolver\",\"DirectReferenceSource\",\"FramePlanBuilder\",\"FrameOrchestrator\",\"SkiaHeadlessBackend\"],\"canonical_ids\":";jsonArray(e,decoded.snapshot->objects,[](auto& o,const auto& r){o<<lowId(r.id);});e<<",\"canonical_kinds\":";jsonKinds(e,decoded.snapshot->objects);e<<",\"scene_ids\":";jsonArray(e,scene->read().records(),[](auto& o,const auto& r){o<<lowId(r.objectId);});e<<",\"scene_kinds\":";jsonSceneKinds(e,scene->read().records());e<<",\"runtime_ids\":";jsonArray(e,coordinator.runtimeScene().records(),[](auto& o,const auto& r){o<<lowId(r.objectId);});e<<",\"runtime_kinds\":";jsonKinds(e,coordinator.runtimeScene().records());e<<",\"runtime_group_findable\":"<<(coordinator.runtimeScene().find(groupId) != nullptr ? "true" : "false")<<",\"visibility_ids\":";jsonIds(e,vis.value().backToFront);e<<",\"draw_ids\":";jsonArray(e,draw.value().items,[](auto& o,const auto& r){o<<lowId(r.objectId);});e<<",\"reference_ids\":";jsonArray(e,refs.value().entries,[](auto& o,const auto& r){o<<lowId(r.record.objectId);});e<<",\"backend_ids\":";jsonIds(e,backend.backendIds());e<<",\"reference_kinds\":";jsonReferenceKinds(e,refs.value().entries);e<<",\"backend_kinds\":";jsonArray(e,obs.traversedKinds,[](auto& o,const auto& kind){o<<'\"'<<kindName(kind)<<'\"';});e<<",\"group_absent_from_pixel_stages\":"<<(groupAbsentFromPixelStages ? "true" : "false")<<",\"object_ids\":[501,502,503,504,505,506,507,508,509],\"object_kinds\":[\"Shape\",\"Image\",\"VectorPath\",\"RichText\",\"VectorStroke\",\"DabStroke\",\"Connector\",\"Sticky\",\"Group\"],\"raster\":{\"width\":256,\"height\":256,\"bytes\":"<<obs.rgba.size()<<",\"fnv1a64\":\""<<obs.digest<<"\",\"sha256\":\""<<sha256(obs.rgba)<<"\"},\"golden_sha256\":\""<<sha256(obs.rgba)<<"\",\"vector_stroke\":{\"object_id\":505,\"mask_id\":1501,\"mask_local_bounds\":[8,-4,16,4],\"mask_world_bounds\":[16,44,24,52],\"render_strip\":[8,46,32,50],\"retained_samples\":[[8,46],[24,46]],\"erased_sample\":[16,46]},\"group\":{\"object_id\":509,\"contributes_pixels\":false},\"plan_digest\":\""<<refs.value().digest<<"\"}\n";}fs::rename(stage,output,ec);if(ec){fs::remove_all(stage,ec);return fail("publication failed");}return 0;
}
