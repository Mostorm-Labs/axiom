...
| G1 | Validating | `GT-G1-01`、`GT-G1-01R`、`GT-G1-02`、`GT-G1-03` 已通过；`GT-G1-04-A` Contract Matrix 已完成，但因无状态结构验证 authority 缺口为 `Blocked`；其余任务未开始 | 需先发布 G1-04-A 的最小 authority closure；之后仍需要审阅式 15-operation apply corpus。G1 Gate 尚未满足整体退出条件。 |
...
| G1/Task 4 | `GT-G1-04` | Implement normalize, validation, idempotency and ApplyPlan | R1,R2 | REQ-OBJ, REQ-EDIT, REQ-INK, REQ-TEXT | D-G1；ADR-0025、ADR-0026；07-03 Operation → Semantic Document | GT-G1-02, GT-G1-03 | Modify | Blocked | Not Started | Blocked | [authority/task reconciliation](GT_G1_04_AUTHORITY_RECONCILIATION.md)；[A Contract Matrix](GT_G1_04_A_CONTRACT_MATRIX.md)；`verification/evidence/gates/G1/<commit>/GT-G1-04/`（未产生） | A 还缺 schema/payload version、repeated payload、kind_version 和 leaf structural closure；C 还缺审阅式 15-operation golden intent、stage/path/outcome。两者均不能由 production codec bless。 | Blocked |
