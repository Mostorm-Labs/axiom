...
| G1 | Validating | `GT-G1-01`、`GT-G1-01R`、历史 `GT-G1-02`、`GT-G1-03` 已通过；`GT-G1-02R` 正在完成当前 machine projection 的 hosted Evidence；其余任务未开始 | `GT-G1-04-A` 必须等待 `GT-G1-02R` 审核并集成到 main；G1 Gate 仍未满足整体退出条件 |
...
| G1/Task 4 | `GT-G1-04` | Implement normalize, validation, idempotency and ApplyPlan | R1,R2 | REQ-OBJ, REQ-EDIT, REQ-INK, REQ-TEXT | D-G1 | GT-G1-02, GT-G1-03 | Conflict | Not Started | Not Started | Not Started | `verification/evidence/gates/G1/<commit>/GT-G1-04/ (planned)` | NOT AUTHORIZED. Notion direct plan omits Normalize and Idempotency; use confirmed operation-only baseline only after explicit authorization. | Not Started |
