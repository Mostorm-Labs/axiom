...
| G1 | Validating | `GT-G1-01`、`GT-G1-01R`、历史 `GT-G1-02`、`GT-G1-03` 与 `GT-G1-02R` 已通过；`GT-G1-02R` 是当前 machine projection 的 `PASS_REVIEW_READY` reconciliation；G1-04-A 语义 authority 已依据批准 token 发布，A0–A3 进入实现对账；其余任务未开始 | `GT-G1-04-A` authority 已闭合，但实现/验证尚未通过；G1 Gate 仍未满足整体退出条件 |
...
| G1/Task 4 | `GT-G1-04` | Implement normalize, validation, idempotency and ApplyPlan | R1,R2 | REQ-OBJ, REQ-EDIT, REQ-INK, REQ-TEXT | D-G1 | GT-G1-02, GT-G1-03 | Modify | Not Started | Not Started | Not Started | `verification/evidence/gates/G1/<commit>/GT-G1-04/ (planned)` | `GT-G1-04-A` authority 已发布；A0–A3 实现对账待开始。GT-G1-04-B/C 仍未开始，GT-G1-04 overall 不得标记 Pass。 | Not Started |
