...
| G1 | Blocked | `GT-G1-01`、`GT-G1-01R`、历史 `GT-G1-02`、`GT-G1-02R` 与 `GT-G1-03` 已通过并保留 lineage；`GT-G1-02R` 已集成为当前 RichText/Stroke machine projection baseline；`GT-G1-04-A` 为 `BLOCKED_AUTHORITY`，B 尚未开始，C 等待 verification materialization | G1 overall 仍未通过；须先完成 Operation/version、ObjectKind version、leaf structural 和 reviewed operation-corpus authority closure，禁止进入 G1-04 产品实现或 GT-G1-05。 |
