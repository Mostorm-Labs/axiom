...
| G1 | Validating | `GT-G1-01`、`GT-G1-01R`、历史 `GT-G1-02`、`GT-G1-03` 与 `GT-G1-02R` 已通过；`GT-G1-02R` 是当前 machine projection 的 `PASS_REVIEW_READY` reconciliation，历史 `GT-G1-02` 仍为其原 machine baseline 的 Pass，但不再代表当前 projection；其余任务未开始 | `GT-G1-04-A` 保持 `BLOCKED_ON_GT_G1_02R_MAIN_INTEGRATION`；G1 Gate 仍未满足整体退出条件 |
...
| G1/Task 4 | `GT-G1-04` | Implement normalize, validation, idempotency and ApplyPlan | R1,R2 | REQ-OBJ, REQ-EDIT, REQ-INK, REQ-TEXT | D-G1 | GT-G1-02, GT-G1-03 | Conflict | Not Started | Not Started | Not Started | `verification/evidence/gates/G1/<commit>/GT-G1-04/ (planned)` | `GT-G1-04-A` remains `BLOCKED_ON_GT_G1_02R_MAIN_INTEGRATION`; GT-G1-04 overall is not passed. After integration, its authority must still be explicitly reconciled before implementation. | Not Started |
| G1/Task 5 | `GT-G1-05` | Implement SemanticDocument atomic apply and ChangeSet | R1,R2 | REQ-OBJ, REQ-EDIT, REQ-INK, REQ-TEXT | D-G1 | GT-G1-04 | Missing | Not Started | Not Started | Not Started | `verification/evidence/gates/G1/<commit>/GT-G1-05/ (planned)` | NOT AUTHORIZED. It cannot begin before GT-G1-04 has completed its authority reconciliation, implementation and validation. | Not Started |
