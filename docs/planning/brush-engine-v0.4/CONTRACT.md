...
### Frozen R1 adapter mapping and legacy boundary

The C2 Vector package is the sole source for new authoring. The adapter builds
`StrokeOptions` with this exact field map:

| ResolvedBrushState field | R1 `StrokeOptions` field | Rule |
| --- | --- | --- |
| `size` | `size` | direct binary64 value |
| `thinning` | `thinning` | direct binary64 value |
| `smoothing` | `smoothing` | direct binary64 value |
| `streamline` | `streamline` | direct binary64 value |
| `pressureSource=simulated` | `simulate_pressure=true` | device pressure is absent |
| `pressureSource=device` | `simulate_pressure=false` | normalized pressure is present |
| `startCap` | `start_cap` | direct boolean value |
| `endCap` | `end_cap` | direct boolean value |
| profile taper values | `start_taper=0`, `end_taper=0` | fixed for profile 1 |
| profile taper values | all taper-enabled/full-length flags false | fixed for profile 1 |
| preview lifecycle | `last=false` | never an authoring field |
| seal lifecycle | `last=true` | never an authoring field |

`easing=linear` is represented by the absence of any non-linear R1 option. No
outer pressure curve, streamlining, smoothing, seed transform, or runtime
override changes these values. The resolved snapshot is frozen at `begin`; only
the lifecycle value of `last` changes between preview and seal.

The existing `BrushDefinition` v1 is outside the C2 new-authoring boundary and
has no production mapping into R1. Its fields are classified explicitly: `family`,
`definitionId`, and `version` are legacy identity; `nominalSize`, `opacity`,
`spacing`, pressure influences, tilt influences, and resource IDs are legacy
parameters/resources. None may be substituted for a ResolvedBrushState field.
If a new-authoring path presents any `BrushDefinition` v1 value, it rejects
before `begin` with an unsupported-legacy-definition error. V1 documents retain
the original definition for read-only decode/render only; no compatibility facade
constructs R1 options and no V1 writer is exposed.
