# ObjectKind Version Registry V1 Release v0.1（提案）

> 状态：**PROPOSED / NOT CURRENT / NOT MANIFEST AUTHORITY**
>
> 这是 `ObjectRecord.kind_id` / `kind_version` 的最小 gap closure 提案，不是 ShapeKind、
...
tag 或当前 C++ enum 都不足以自行推导 version policy；本提案只提出该缺失表。
...
`G104-S08`：**PROPOSED UNTIL HUMAN APPROVAL**。所有 released V1 ObjectKind 的
...
该提案的 fail-closed 行为使未来 kind 的发布成为显式 schema evolution：先发布新 registry
