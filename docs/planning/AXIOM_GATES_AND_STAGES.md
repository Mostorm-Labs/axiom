...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| AR-0 | 对账唯一路线、来源、冲突和追踪链；不实现产品代码或 ABI | 仓库文档、已登录 Notion 来源、CI 文档检查 | 用户确认路线、Notion capture、现行 ADR/Contract | 动态 Notion 无不可变 revision；R5-B 权威任务清单缺失 | 后续 Gate 仍有独立 RFC/任务阻塞，但 AR-0 已通过 | R1～R5 追踪基线 | Markdown/link/fence/privacy、任务枚举、Route/DL 映射、commit 与 artifact hash | `Pass` |
| G0 | 建立统一 conformance/corpus/runner/report；不冻结产品语义或协作算法 | Headless、Web、Windows、Android、iOS/iPadOS；macOS core conformance | AR-0 PASS（已满足） | Gate Report schema 的具体版本需 G0 冻结 | `GT-G0-00` 尚未开始 | R1 | E1/E2 protocol vectors、E3 report aggregation、适用平台 harness artifact | `Not Started` |
