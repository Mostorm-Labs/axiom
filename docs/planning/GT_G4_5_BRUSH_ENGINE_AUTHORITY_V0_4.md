...
The precise mapping is frozen in the contract: C2 ResolvedBrushState fields map
directly to R1 StrokeOptions, with `last` controlled only by preview/seal
lifecycle. Existing BrushDefinition v1 fields have no production mapping into
R1; new authoring rejects that legacy definition before begin. V1 documents keep
the original definition for read-only decode/render only, with no compatibility
facade or V1 writer.
