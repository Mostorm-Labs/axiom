...
| 视觉文档运行时 | Visual Document Runtime | 负责 Document、Operations、EditorSession、RichText、Ink、Scene 与 Rendering 等共享语义和执行边界。数据库、文件、网络、认证和 transport 的物理 adapter 位于外部 ports；Shared Data Runtime 负责数据侧编排，但语言、物理 owner 和 Bridge 仍待 RFC。 | 核心定位与职责边界现行；物理实现待审 |
...
| Arc | Arc | 与 Axiom 同仓、可独立构建和抽取的 input-to-display 模块；通过版本化协议连接平台输入与 Axiom Preview Model，拥有独立 Preview Presentation Target，不拥有 Document、Canonical RenderTarget 或 Canonical 语义。Web、Windows RNW、Android RN、iOS/iPadOS RN 都必须实现产品 Arc；ChromiumOS 继承 Web Arc。任一 backend 失败都自动使用 Canonical-only，不得丢 confirmed input、取消 Operation、改变 digest 或阻断保存/恢复。 | 产品能力、模块边界与 fallback 现行；packaging、SDK、ABI、surface/backend 和延迟阈值待后续 Contract/Gate |
| FastInk | FastInk | 低延迟输入到显示的能力主题及 POC-06 名称，不再用作 Arc 物理模块名。系统级 Raw Input/DRM FastInk 是独立条件式预研。 | 术语分工现行，ADR-0024 |
| 共享数据运行时 | Shared Data Runtime | Axiom 外部的数据侧编排边界，负责 DocumentSession、Page repository custody、storage/checkpoint、offline/outbox、sync、blob 和 suspend/resume；不拥有 Page Collection 的产品语义/生命周期，也不参与 Pointer/Render 热路径。数据侧职责方向已由 ADR-0025 接受；具体语言（包括 TypeScript）、包拆分和 Bridge 仍由后续 RFC 冻结。 | 现行方向；物理实现待 G7/G8 RFC |
...
| 页面集合 | Page Collection | 产品中管理多个 Page/Document 引用、名称、顺序、导航和生命周期的上层集合。每个 Page 恰对应一个独立 Document；集合不是 Document 内的 synthetic root。产品语义 owner 已固定为上层产品层；repository schema、Data Runtime custody、保存和同步协议仍待后续决策。 | 一 Page 一 Document 与产品层 owner 已确认；物理方案开放 |
...
| 原子 Operation apply 边界 | Atomic Operation Apply Boundary / 历史称 `Document Transaction` | 对一个 Operation 执行完整验证并原子发布其全部语义修改的内部边界。Operation 可以携带多对象或领域内的原子 payload，但不存在独立的 canonical `Transaction → operations[]` 外层；该历史术语不能成为 Sync、Replay、Undo 或 wire 的第二种事实。 | 现行规范，ADR-0025 |
| 操作批次 | Operation Batch | C ABI 或 transport 一次携带的版本化 Operation 容器；一个 batch 不自动等于原子 apply、一个用户意图或一个协作因果单位。 | 公共 port 现行；schema 待审 |
| 变更集 | `ChangeSet` | 成功 Operation apply 针对明确 before/after revision 派生的 Scene 更新输入，不是第二份持久化事实。 | 现行规范，ADR-0019 |
| 语义变化 | `SemanticChanges` | ChangeSet 中由 Operation apply 结果确定的创建、删除、属性、层级、order 和 resource binding 变化；可用于增量正确性校验。 | 现行规范，ADR-0019 |
...
| 版本历史 | Version History | 用户可浏览、预览或恢复的持久文档版本产品能力。它不等于 EditorSession Undo/Redo、内部 Operation Log、autosave checkpoint 或 Snapshot codec；是否进入 V1 及其恢复语义仍待第三组确认。 | 需求候选 |
...
| 外部表面 | `ExternalSurface` / `ExternalSurfaceRegistry` | 产品路线中的受控 Overlay placement：Document/RuntimeScene 只持稳定 ID、geometry、clip、z-order 和 lifecycle 语义，真实 WebView/Video/native surface 由 Platform Host/Shell registry 持有；不允许 native handle 进入 Document。 | 产品边界现行，G6 实现；ADR-0023 |
...
| Arc 输入源 | Arc Input Source | 平台侧批量采集高频输入并报告 source capabilities/loss 的组件；source loss 与 preview presentation failure 是不同 failure domain。所有产品目标必须有符合平台能力的输入/Preview 路径；Windows、Android、iOS/iPadOS 不逐 sample 经过 RN JS，Web 使用薄批量 adapter。失败按 Canonical-only 处理。 | 产品平台义务与 Arc 边界现行；具体 adapter ABI/capability/延迟门禁待审 |
...
| 文档已持久化 | Document Durable | Document 的 Operation/checkpoint 已满足声明的本地 durability contract；不表示全部 Blob 已在本地可用，也不表示远端已确认。 | 需求边界候选 |
| 资源可离线使用 | Resources Available Offline | 恢复或打开所需的 Blob 已在本地存在并通过 ContentHash 校验；它与 Document Durable 分开报告。 | 需求边界候选 |
...
| 中文首选名 | 英文或代码名 | 当前工作定义 | 规范地位 |
| --- | --- | --- | --- |
| 产品一级平台 | Product Tier A | Web、Windows RNW、Android RN、iOS RN 和 iPadOS RN；承担 V1 正式产品 Shell、完整用户流、性能、IME/Input/Surface、发布和支持门禁。 | 现行规范，ADR-0025 |
| 历史可移植性二级平台 | Portability Tier B | ADR-0015 早期快照中的 macOS、iOS、iPadOS 分级。当前 iOS/iPadOS 已是 Product Tier A；macOS 为 Deferred/Web + core conformance。现行文档不得把该历史标签当作当前产品分级。 | 历史术语；ADR-0025 已替代 |
...
| Intent/Command / Operation / Atomic Operation Apply / Operation Batch / ChangeSet | 分别是用户意图、唯一 canonical 语义修改、单 Operation 原子发布边界、传输容器和提交后的派生变化。 |
