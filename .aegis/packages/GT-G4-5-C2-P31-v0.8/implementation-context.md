# GT-G4-5-C2 P31 v0.8 implementation context
...
## v0.8 RED oracle materialization

The executable anchor oracle is materialized at `apps/ink_playground/tests/c2_red_oracle_test.py` and bound by exact SHA-256 in package.json. It runs only against the legacy task-anchor surface, records raw command observations, and must exit 0 with every frozen pre-change gap RED before production mutation.
